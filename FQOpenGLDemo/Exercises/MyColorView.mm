...
//  FQOpenGLDemo
//
//  Created by fan qi on 2018/7/22.
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        [self renderWithVertexFileName:@"color" fragmentFileName:@"color"];
    }
    return self;
}
...
- (void)renderWithVertexFileName:(NSString *)vertexName fragmentFileName:(NSString *)fragmentName {
    ...
    // 物体
    GLuint objProgram = [FQShaderHelper linkShaderWithVertexFileName:vertexName fragmentFileName:fragmentName];
    if (objProgram == 0) {
        return;
    }
    ...
    // 光源
    GLuint lampProgram = [FQShaderHelper linkShaderWithVertexFileName:vertexName fragmentFileName:@"lamp"];
    if (lampProgram == 0) {
        return;
    }
    
    {
        // 使用和物体同一个顶点着色器（注释这段也可以正常显示）
        // we only need to bind to the VBO (to link it with glVertexAttribPointer), no need to fill it; the VBO's data already contains all we need (it's already bound, but we do it again for educational purposes
        glBindBuffer(GL_ARRAY_BUFFER, VBO);
        
        GLuint posLoc2 = glGetAttribLocation(objProgram, "aPos");
        glEnableVertexAttribArray(posLoc2);
        glVertexAttribPointer(posLoc2, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(GL_FLOAT), (void *)0);
    }
